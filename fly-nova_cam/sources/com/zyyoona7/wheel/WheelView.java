...
public class WheelView<T> extends View implements Runnable {
    ...
    private void initAttrsAndDefault(Context context, AttributeSet attributeSet) {
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C0605R.styleable.WheelView);
        this.mTextSize = typedArrayObtainStyledAttributes.getDimension(C0605R.styleable.WheelView_wv_textSize, DEFAULT_TEXT_SIZE);
        this.isAutoFitTextSize = typedArrayObtainStyledAttributes.getBoolean(C0605R.styleable.WheelView_wv_autoFitTextSize, false);
        this.mTextAlign = typedArrayObtainStyledAttributes.getInt(C0605R.styleable.WheelView_wv_textAlign, 1);
        int i = C0605R.styleable.WheelView_wv_textBoundaryMargin;
        float f = DEFAULT_TEXT_BOUNDARY_MARGIN;
        this.mTextBoundaryMargin = typedArrayObtainStyledAttributes.getDimension(i, f);
        this.mTextColor = typedArrayObtainStyledAttributes.getColor(C0605R.styleable.WheelView_wv_normalItemTextColor, DEFAULT_NORMAL_TEXT_COLOR);
        this.mSelectedItemTextColor = typedArrayObtainStyledAttributes.getColor(C0605R.styleable.WheelView_wv_selectedItemTextColor, -16777216);
        this.mLineSpacing = typedArrayObtainStyledAttributes.getDimension(C0605R.styleable.WheelView_wv_lineSpacing, DEFAULT_LINE_SPACING);
        this.isIntegerNeedFormat = typedArrayObtainStyledAttributes.getBoolean(C0605R.styleable.WheelView_wv_integerNeedFormat, false);
        String string = typedArrayObtainStyledAttributes.getString(C0605R.styleable.WheelView_wv_integerFormat);
        this.mIntegerFormat = string;
        if (TextUtils.isEmpty(string)) {
            this.mIntegerFormat = DEFAULT_INTEGER_FORMAT;
        }
        int i2 = typedArrayObtainStyledAttributes.getInt(C0605R.styleable.WheelView_wv_visibleItems, 5);
        this.mVisibleItems = i2;
        this.mVisibleItems = adjustVisibleItems(i2);
        int i3 = typedArrayObtainStyledAttributes.getInt(C0605R.styleable.WheelView_wv_selectedItemPosition, 0);
        this.mSelectedItemPosition = i3;
        this.mCurrentScrollPosition = i3;
        this.isCyclic = typedArrayObtainStyledAttributes.getBoolean(C0605R.styleable.WheelView_wv_cyclic, false);
        this.isShowDivider = typedArrayObtainStyledAttributes.getBoolean(C0605R.styleable.WheelView_wv_showDivider, false);
        this.mDividerType = typedArrayObtainStyledAttributes.getInt(C0605R.styleable.WheelView_wv_dividerType, 0);
        this.mDividerSize = typedArrayObtainStyledAttributes.getDimension(C0605R.styleable.WheelView_wv_dividerHeight, DEFAULT_DIVIDER_HEIGHT);
        this.mDividerColor = typedArrayObtainStyledAttributes.getColor(C0605R.styleable.WheelView_wv_dividerColor, -16777216);
        this.mDividerPaddingForWrap = typedArrayObtainStyledAttributes.getDimension(C0605R.styleable.WheelView_wv_dividerPaddingForWrap, f);
        this.mDividerOffset = typedArrayObtainStyledAttributes.getDimensionPixelOffset(C0605R.styleable.WheelView_wv_dividerOffset, 0);
        this.isDrawSelectedRect = typedArrayObtainStyledAttributes.getBoolean(C0605R.styleable.WheelView_wv_drawSelectedRect, false);
        this.mSelectedRectColor = typedArrayObtainStyledAttributes.getColor(C0605R.styleable.WheelView_wv_selectedRectColor, 0);
        this.isCurved = typedArrayObtainStyledAttributes.getBoolean(C0605R.styleable.WheelView_wv_curved, true);
        this.mCurvedArcDirection = typedArrayObtainStyledAttributes.getInt(C0605R.styleable.WheelView_wv_curvedArcDirection, 1);
        this.mCurvedArcDirectionFactor = typedArrayObtainStyledAttributes.getFloat(C0605R.styleable.WheelView_wv_curvedArcDirectionFactor, 0.75f);
        float f2 = typedArrayObtainStyledAttributes.getFloat(C0605R.styleable.WheelView_wv_curvedRefractRatio, 0.9f);
        float fMin = typedArrayObtainStyledAttributes.getFloat(C0605R.styleable.WheelView_wv_refractRatio, DEFAULT_REFRACT_RATIO);
        this.mRefractRatio = fMin;
        if (this.isCurved) {
            fMin = Math.min(f2, fMin);
        }
        this.mRefractRatio = fMin;
        if (fMin > DEFAULT_REFRACT_RATIO || fMin < 0.0f) {
    ...
    private void initDefaultVolume(Context context) {
        if (((AudioManager) context.getSystemService("audio")) != null) {
            this.mSoundHelper.setPlayVolume((r4.getStreamVolume(3) * DEFAULT_REFRACT_RATIO) / r4.getStreamMaxVolume(3));
            return;
        }
    ...
    protected void onMeasure(int i, int i2) {
        int paddingTop;
        if (this.isCurved) {
            paddingTop = (int) ((((this.mItemHeight * this.mVisibleItems) * 2) / 3.141592653589793d) + getPaddingTop() + getPaddingBottom());
        } else {
            paddingTop = (this.mItemHeight * this.mVisibleItems) + getPaddingTop() + getPaddingBottom();
        }
    ...
    protected void onSizeChanged(int i, int i2, int i3, int i4) {
        ...
        calculateDrawStart();
        calculateLimitY();
        int iCalculateItemDistance = calculateItemDistance(this.mSelectedItemPosition);
        if (iCalculateItemDistance > 0) {
            doScroll(iCalculateItemDistance);
        }
    }
    ...
    */
    protected void onDraw(Canvas canvas) {
        ...
        drawDivider(canvas);
        int iDividedItemHeight = this.mScrollOffsetY / dividedItemHeight();
        int iDividedItemHeight2 = this.mScrollOffsetY % dividedItemHeight();
        int i3 = (this.mVisibleItems + 1) / 2;
        if (iDividedItemHeight2 < 0) {
            i = (iDividedItemHeight - i3) - 1;
        } else {
            if (iDividedItemHeight2 > 0) {
                i = iDividedItemHeight - i3;
                i2 = iDividedItemHeight + i3 + 1;
                while (i < i2) {
                    if (this.isCurved) {
                        draw3DItem(canvas, i, iDividedItemHeight2);
                    } else {
                        drawItem(canvas, i, iDividedItemHeight2);
                    }
                    i++;
                }
            }
    ...
    private void drawItem(Canvas canvas, int i, int i2) {
        ...
        int iDividedItemHeight = ((i - (this.mScrollOffsetY / dividedItemHeight())) * this.mItemHeight) - i2;
        int i3 = this.mStartX;
        int iRemeasureTextSize = this.isAutoFitTextSize ? remeasureTextSize(dataByIndex) : this.mCenterToBaselineY;
        if (Math.abs(iDividedItemHeight) <= 0) {
            this.mPaint.setColor(this.mSelectedItemTextColor);
            clipAndDraw2DText(canvas, dataByIndex, this.mSelectedItemTopLimit, this.mSelectedItemBottomLimit, iDividedItemHeight, iRemeasureTextSize);
        } else if (iDividedItemHeight > 0 && iDividedItemHeight < this.mItemHeight) {
            this.mPaint.setColor(this.mSelectedItemTextColor);
            clipAndDraw2DText(canvas, dataByIndex, this.mSelectedItemTopLimit, this.mSelectedItemBottomLimit, iDividedItemHeight, iRemeasureTextSize);
            ...
            clipAndDraw2DText(canvas, dataByIndex, this.mSelectedItemBottomLimit, this.mClipBottom, iDividedItemHeight, iRemeasureTextSize);
            ...
            changeTypefaceIfBoldForSelectedItem();
            clipAndDraw2DText(canvas, dataByIndex, this.mClipTop, this.mSelectedItemTopLimit, iDividedItemHeight, iRemeasureTextSize);
            ...
            changeTypefaceIfBoldForSelectedItem();
            clipAndDraw2DText(canvas, dataByIndex, this.mClipTop, this.mClipBottom, iDividedItemHeight, iRemeasureTextSize);
    ...
    private int remeasureTextSize(String str) {
        ...
        while (fMeasureText > f) {
            f3 -= DEFAULT_REFRACT_RATIO;
            if (f3 <= 0.0f) {
                break;
            }
            this.mPaint.setTextSize(f3);
            fMeasureText = this.mPaint.measureText(str);
        }
    ...
    private void draw3DItem(Canvas canvas, int i, int i2) {
        ...
        int iDividedItemHeight = ((i - (this.mScrollOffsetY / dividedItemHeight())) * this.mItemHeight) - i2;
        double d = height;
        if (Math.abs(iDividedItemHeight) > (3.141592653589793d * d) / 2.0d) {
            return;
        }
        double d2 = iDividedItemHeight / d;
        float degrees = (float) Math.toDegrees(-d2);
        float fSin = (float) (Math.sin(d2) * d);
        float fCos = (float) ((1.0d - Math.cos(d2)) * d);
        int iCos = (int) (Math.cos(d2) * 255.0d);
        int i3 = this.mStartX;
        int iRemeasureTextSize = this.isAutoFitTextSize ? remeasureTextSize(dataByIndex) : this.mCenterToBaselineY;
        if (Math.abs(iDividedItemHeight) <= 0) {
            this.mPaint.setColor(this.mSelectedItemTextColor);
            this.mPaint.setAlpha(255);
            clipAndDraw3DText(canvas, dataByIndex, this.mSelectedItemTopLimit, this.mSelectedItemBottomLimit, degrees, fSin, fCos, iRemeasureTextSize);
        } else if (iDividedItemHeight > 0 && iDividedItemHeight < this.mItemHeight) {
            this.mPaint.setColor(this.mSelectedItemTextColor);
            this.mPaint.setAlpha(255);
            clipAndDraw3DText(canvas, dataByIndex, this.mSelectedItemTopLimit, this.mSelectedItemBottomLimit, degrees, fSin, fCos, iRemeasureTextSize);
            this.mPaint.setColor(this.mTextColor);
            this.mPaint.setAlpha(iCos);
            float textSize = this.mPaint.getTextSize();
            this.mPaint.setTextSize(this.mRefractRatio * textSize);
            changeTypefaceIfBoldForSelectedItem();
            clipAndDraw3DText(canvas, dataByIndex, this.mSelectedItemBottomLimit, this.mClipBottom, degrees, fSin, fCos, recalculateCenterToBaselineY());
            this.mPaint.setTextSize(textSize);
            resetTypefaceIfBoldForSelectedItem();
        } else if (iDividedItemHeight < 0 && iDividedItemHeight > (-this.mItemHeight)) {
            this.mPaint.setColor(this.mSelectedItemTextColor);
            ...
            this.mPaint.setTextSize(this.mRefractRatio * textSize2);
            changeTypefaceIfBoldForSelectedItem();
            clipAndDraw3DText(canvas, dataByIndex, this.mClipTop, this.mSelectedItemTopLimit, degrees, fSin, fCos, recalculateCenterToBaselineY());
            this.mPaint.setTextSize(textSize2);
            resetTypefaceIfBoldForSelectedItem();
        } else {
            this.mPaint.setColor(this.mTextColor);
            ...
            this.mPaint.setTextSize(this.mRefractRatio * textSize3);
            changeTypefaceIfBoldForSelectedItem();
            clipAndDraw3DText(canvas, dataByIndex, this.mClipTop, this.mClipBottom, degrees, fSin, fCos, recalculateCenterToBaselineY());
            this.mPaint.setTextSize(textSize3);
            resetTypefaceIfBoldForSelectedItem();
        }
        if (this.isAutoFitTextSize) {
    ...
    private void draw3DText(Canvas canvas, String str, float f, float f2, float f3, int i) {
        ...
        if (i3 == 0) {
            f4 = (this.mCurvedArcDirectionFactor + DEFAULT_REFRACT_RATIO) * i2;
        } else if (i3 == 2) {
            f4 = i2 * (DEFAULT_REFRACT_RATIO - this.mCurvedArcDirectionFactor);
        }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (actionMasked == 0) {
            ...
            if (Math.abs(yVelocity) > this.mMinFlingVelocity) {
                ...
                int iCalculateDistanceToEndPoint = y + calculateDistanceToEndPoint((this.mScrollOffsetY + y) % dividedItemHeight());
                boolean z = iCalculateDistanceToEndPoint < 0 && this.mScrollOffsetY + iCalculateDistanceToEndPoint >= this.mMinScrollY;
                boolean z2 = iCalculateDistanceToEndPoint > 0 && this.mScrollOffsetY + iCalculateDistanceToEndPoint <= this.mMaxScrollY;
                if (z || z2) {
                    this.mScroller.startScroll(0, this.mScrollOffsetY, 0, iCalculateDistanceToEndPoint);
                }
            }
    ...
    private int getCurrentPosition() {
        int iDividedItemHeight;
        if (this.mDataList.isEmpty()) {
            return -1;
        }
        int i = this.mScrollOffsetY;
        if (i < 0) {
            iDividedItemHeight = (i - (this.mItemHeight / 2)) / dividedItemHeight();
        } else {
            iDividedItemHeight = (i + (this.mItemHeight / 2)) / dividedItemHeight();
        }
        int size = iDividedItemHeight % this.mDataList.size();
    ...
    public void setSelectedItemPosition(int i, boolean z, int i2) {
        int iCalculateItemDistance;
        if (isPositionInRange(i) && (iCalculateItemDistance = calculateItemDistance(i)) != 0) {
            abortFinishScroll();
            if (z) {
                Scroller scroller = this.mScroller;
                ...
                scroller.startScroll(0, i3, 0, iCalculateItemDistance, i2);
                invalidateIfYChanged();
                ViewCompat.postOnAnimation(this, this);
                return;
            }
  ...
  0x000c: PHI (r0v4 float) = (r0v2 float), (r0v3 float) binds: [B:6:0x000a, B:9:0x0012] A[DONT_GENERATE, DONT_INLINE]] */
    ...
    */
    ...
    public void setRefractRatio(float f) {
        ...
        if (f > DEFAULT_REFRACT_RATIO || f < 0.0f) {
            this.mRefractRatio = DEFAULT_REFRACT_RATIO;
        }
