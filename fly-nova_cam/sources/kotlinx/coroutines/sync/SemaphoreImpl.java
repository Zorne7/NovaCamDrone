...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM2095constructorimpl;
            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM2095constructorimpl = SegmentOrClosed.m2095constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM2095constructorimpl = SegmentOrClosed.m2095constructorimpl(segment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m2100isClosedimpl(objM2095constructorimpl)) {
                Segment segmentM2098getSegmentimpl = SegmentOrClosed.m2098getSegmentimpl(objM2095constructorimpl);
                while (true) {
                    Segment segment3 = (Segment) this.tail;
                    if (segment3.getId() >= segmentM2098getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!segmentM2098getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (C0141xc40028dd.m78m(tail$FU, this, segment3, segmentM2098getSegmentimpl)) {
                        if (segment3.decPointers$kotlinx_coroutines_core()) {
                            segment3.remove();
                        }
                    } else if (segmentM2098getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM2098getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m2098getSegmentimpl(objM2095constructorimpl);
        int i = (int) (andIncrement % SemaphoreKt.SEGMENT_SIZE);
        if (!ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m464m(semaphoreSegment.acquirers, i, null, cont)) {
            if (ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m464m(semaphoreSegment.acquirers, i, SemaphoreKt.PERMIT, SemaphoreKt.TAKEN)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m2100isClosedimpl(objM2095constructorimpl)) {
                ...
                while (true) {
                    ...
                    if (C0141xc40028dd.m78m(head$FU, this, segment3, segmentM2098getSegmentimpl)) {
                        if (segment3.decPointers$kotlinx_coroutines_core()) {
                            segment3.remove();
                        }
                    } else if (segmentM2098getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m464m(semaphoreSegment.acquirers, i2, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
        }
