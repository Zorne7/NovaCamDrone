...
public final class DebugProbesImpl {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        if (objNewInstance != null) {
            objM558constructorimpl = Result.m558constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(objNewInstance, 1));
            return (Function1) (Result.m564isFailureimpl(objM558constructorimpl) ? null : objM558constructorimpl);
        }
    ...
    private final <R> List<R> dumpCoroutinesInfoImpl(final Function2<? super CoroutineOwner<?>, ? super CoroutineContext, ? extends R> create) {
        ...
        try {
            ...
            return SequencesKt.toList(SequencesKt.mapNotNull(SequencesKt.sortedWith(CollectionsKt.asSequence(debugProbesImpl.getCapturedCoroutines()), new C0799x8808a3e8()), new Function1<CoroutineOwner<?>, R>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpCoroutinesInfoImpl$1$3
        ...
        try {
            ...
            for (CoroutineOwner coroutineOwner : SequencesKt.sortedWith(SequencesKt.filter(CollectionsKt.asSequence(debugProbesImpl.getCapturedCoroutines()), new Function1<CoroutineOwner<?>, Boolean>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpCoroutinesSynchronized$1$2
                ...
                if (Intrinsics.areEqual(debugCoroutineInfoImpl.get_state(), DebugCoroutineInfoImplKt.RUNNING) && listEnhanceStackTraceWithThreadDumpImpl == listLastObservedStackTrace) {
                    strStringPlus = Intrinsics.stringPlus(debugCoroutineInfoImpl.get_state(), " (Last suspension stacktrace, not an actual stacktrace)");
                } else {
                    strStringPlus = debugCoroutineInfoImpl.get_state();
                }
                out.print("\n\nCoroutine " + coroutineOwner.delegate + ", state: " + strStringPlus);
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        try {
            Result.Companion companion = Result.INSTANCE;
            DebugProbesImpl debugProbesImpl = this;
            objM558constructorimpl = Result.m558constructorimpl(thread.getStackTrace());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            objM558constructorimpl = Result.m558constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m564isFailureimpl(objM558constructorimpl)) {
            objM558constructorimpl = null;
        }
        StackTraceElement[] stackTraceElementArr = (StackTraceElement[]) objM558constructorimpl;
    ...
    private final Pair<Integer, Integer> findContinuationStartIndex(int indexOfResumeWith, StackTraceElement[] actualTrace, List<StackTraceElement> coroutineTrace) {
        ...
        while (i < 3) {
            ...
            if (iFindIndexOfFrame != -1) {
                return TuplesKt.m429to(Integer.valueOf(iFindIndexOfFrame), Integer.valueOf(i));
            }
            i = i2;
        }
        return TuplesKt.m429to(-1, 0);
    }
            ...
            if (debugProbesImpl.isInstalled$kotlinx_coroutines_core()) {
                ...
                if (debugCoroutineInfoImplRemove == null) {
                    ...
                    if (lastObservedFrame$kotlinx_coroutines_core != null) {
                        coroutineStackFrameRealCaller = debugProbesImpl.realCaller(lastObservedFrame$kotlinx_coroutines_core);
                    }
    ...
    public final List<DebuggerInfo> dumpDebuggerInfo() {
        ...
        try {
            ...
            return SequencesKt.toList(SequencesKt.mapNotNull(SequencesKt.sortedWith(CollectionsKt.asSequence(debugProbesImpl.getCapturedCoroutines()), new C0799x8808a3e8()), new Function1<CoroutineOwner<?>, DebuggerInfo>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpDebuggerInfo$$inlined$dumpCoroutinesInfoImpl$1
                @Override // kotlin.jvm.functions.Function1
